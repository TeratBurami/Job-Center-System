  ...
  return (
    <div className="mt-10">
      ...
      <div className="flex flex-col gap-8 p-6 bg-slate-200 shadow-md rounded-xl shadow-slate-800 w-3/4 md:w-1/3 mx-auto mt-4">
        <div><p>Gmail:</p><input onChangeCapture={(e: React.ChangeEvent<HTMLInputElement>)=>setData({...data,gmail:e.target.value})} className="w-full rounded border border-slate-900" type="text" /></div>
        <div><p>Password:</p><input onChangeCapture={(e: React.ChangeEvent<HTMLInputElement>)=>setData({...data,password:e.target.value})} className="w-full rounded border border-slate-900" type="password" /></div>
